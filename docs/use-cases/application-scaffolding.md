...
- KubeRocketCI instance is [configured](../operator-guide/prerequisites.md) with Gerrit, Tekton, [Argo CD](../operator-guide/argocd-integration.md) and [GitOps](../user-guide/gitops.md) repo.
- Developer has access to the EDP instances using the Single-Sign-On approach.
- Developer has the `Administrator` role in GitHub for GitOps repo (to perform merge).
...
5. Opt for the `Create from template` strategy to scaffold our application from the template provided by the KubeRocketCI and press the `Create` button:
   ...
   ![Codebase Info](../assets/use-cases/fastapi-scaffolding/component-strategy.png "Step codebase strategy")
...
3. Track Pipeline's status by accessing **Tekton Dashboard** by clicking the `fastapi-demo-main-build-zwpj` application link.

    ![Alt text](../assets/use-cases/fastapi-scaffolding/tekton.png "Console logs")
...
6. Go to UI Portal -> `Environments` tab and push the `Create environment` button to create pipeline.

   ![Alt text](../assets/use-cases/fastapi-scaffolding/create-cd-pipeline.png "Console logs")
...
7. Deploy the initial version of the application to the development environment:
    ...
    - In the `Select image tag` select version `0.0.1-SNAPSHOT.1` and push the `Start deploy` button.
...
3. Check that the new version with Ingress is deployed:
    ...
    fastapi-demo   <none>   fastapi-demo-edp-delivery-dn-dev-mypipe-dev.eks-sandbox.aws.main.edp.projects.epam.com   10.100.182.134   80      14m
