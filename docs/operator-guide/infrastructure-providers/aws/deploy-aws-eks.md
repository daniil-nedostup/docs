...
4. (Optional) Create the Kaniko AWS IAM Role:
    ...
    To configure the Kaniko AWS IAM Role, set the `create_iam_kaniko` variable to `true` in the `eks/template.tfvars` file:
    ...
    Once the EKS cluster and the Kaniko AWS IAM Role are successfully created, you can use the resulting **ARN** in the `edp-tekton.kaniko.roleArn` field within the `values.yaml` file during the [KubeRocketCI installation](../../install-kuberocketci.md).
